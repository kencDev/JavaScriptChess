//all pieces can now only do viable moves.
//now time to add 2 player.
//need to change black pawn move set.
...
function pawn (x,y, piece_id) {
    if (el.playerSide == "W") {
        if(x <= 8 && y <= 8 ) {
            ...
            //taking one diagonal pice for paWNS
            else {
                if ( x+1 <= 8 && el.positions["square_id"][(x + 1 ) + ""+  (y + 1)]["piece_id"].charAt(0) == "B" ) {
                    console.log(el.positions["square_id"][(x + 1 ) + ""+  (y + 1)]["piece_id"].charAt(0))
    ...
    if (el.playerSide == "B") {
        ...
        //one move up and diagonal takes
        if (el.positions["square_id"][(x) + ""+  (y - 1)]["piece_id"] == "")
            {el.playerMoves[piece_id].add(x + "" + (y-1));}
        else {
...
function knight (x,y, piece_id) {
        // down
        //(el.positions["square_id"][y + ""+ horizontal]["piece_id"] != "")
        if (y -2 >= 1) {
            //left
            if (x - 1 >= 1  ) {
